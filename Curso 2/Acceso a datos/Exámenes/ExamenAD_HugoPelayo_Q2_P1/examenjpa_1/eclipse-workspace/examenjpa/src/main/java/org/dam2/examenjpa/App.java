...
import org.dam2.menu.Opcion3;
import org.dam2.menu.Opcion4;
...
public class App 
{
	...
	public App() {
		...
		Contexto.getDaoPerec().save((Perecedero) productos.get(0));
		Contexto.getDaoPerec().save((Perecedero) productos.get(1));
		Contexto.getDaoNoPerec().save((NoPerecedero) productos.get(2));
		Contexto.getDaoNoPerec().save((NoPerecedero) productos.get(3));
		
		Contexto.getDaoProveedor().save(proveedores.get(0));
		Contexto.getDaoProveedor().save(proveedores.get(1));
		
		Contexto.getDaoVenta().save(ventas.get(0));
		
		for (Cliente cliente : clientes) { Contexto.getDaoCliente().save(cliente); }
		...
		opciones.add(Opcion3.builder().descripcion("Subir PVP perecederos").build());
		opciones.add(Opcion4.builder().descripcion("Lanzar queries").build());
