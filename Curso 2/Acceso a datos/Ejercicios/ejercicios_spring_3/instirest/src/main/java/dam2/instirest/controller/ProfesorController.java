package dam2.instirest.controller;

import java.util.List;
import java.util.Set;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.http.HttpStatus;
import org.springframework.http.ResponseEntity;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

import dam2.instirest.model.Instituto;
import dam2.instirest.model.Profesor;
import dam2.instirest.service.InstitutoServ;
import dam2.instirest.service.ProfesorServ;

@RestController
@RequestMapping("/instituto/profesores")
public class ProfesorController {
	@Autowired
	ProfesorServ profesorServ;
	
	@GetMapping("consultar")
	ResponseEntity<List<Profesor>> consultar() {
		ResponseEntity<List<Profesor>> respuesta = null;
		
		Set<Profesor> profesores = profesorServ.consultarTodos();
		respuesta = new ResponseEntity<>(profesores.stream().toList(), HttpStatus.OK);
		
		return respuesta;
	}
}
