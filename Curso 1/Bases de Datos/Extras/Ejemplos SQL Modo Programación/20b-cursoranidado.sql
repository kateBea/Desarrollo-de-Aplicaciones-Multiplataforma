...
use curso;
DROP PROCEDURE IF EXISTS anidados2;
DELIMITER //
CREATE PROCEDURE anidados2 () 
BEGIN  
    ...
    recorrer_dptos: loop -- Recorrerlo
		...
		recorrer_empleados: loop
			...
			SELECT CONCAT ("Departamento ", dnum_aux, " - ", dnombre_aux) as NombreDepartamento;
            if ultima_fila=1 then -- Aseguramos que no llegamos a la última fila 
				leave recorrer_empleados;
			end if;
            -- Tratamiento del registro recuperado
            SELECT CONCAT (" Empleado: ", enum_aux, " - ", eapellido_aux) as Empleados;
            set total_empleados = total_empleados + 1;
        end loop recorrer_empleados;
        close c_empleados;
        
        set ultima_fila=0;  
		SELECT CONCAT (" Total empleados: ", total_empleados) as Total; 
