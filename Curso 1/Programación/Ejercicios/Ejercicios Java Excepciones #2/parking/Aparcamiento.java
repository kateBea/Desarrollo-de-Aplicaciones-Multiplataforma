...
public final class Aparcamiento {
    ...
    public void introducirVehiculo(Vehiculo v) throws AparcamientoException {
        ...
        if (m_Capacidad != 0) {
            ...
            if (!encontrado) {
                ...
                throw new AparcamientoException(String.format("El vehículo con matrícula [ %s ] a ser aparcado ya existe", v.getMatricula()));
        }
        else 
            throw new AparcamientoException("No hay plazas suficientes");
    }
     ...
     */
    public double sacarVehiculo(Vehiculo v) throws AparcamientoException {
        // se asume que el vehículo no existe porque se va buscar
        boolean encontrado = false;
        double importe = -1.0;
        int indice = 0;
        ...
        if (encontrado) {
            ...
            throw new AparcamientoException(String.format("El vehículo con matrícula [ %s ] a ser retirado no existe", v.getMatricula()));
     ...
     * @return Devuelve el importe a cobrar por sacar el vehículo
     */
    public double sacarVehiculo(String matricula) throws AparcamientoException {
        // se asume que el vehículo no existe porque se va buscar
        boolean encontrado = false;
        double importe = -1.0;
        int indice = 0;
        ...
        throw new AparcamientoException(String.format("El vehículo con matrícula [ %s ] a ser retirado no existe", matricula));
